...
description: "Karamihan ng take-home tests, fail dahil sa setup friction, malabong brief, o hindi nire-respeto ang oras. Ganito ko dinesign ang gustong gawin ng kandidato."
...
Kino-clone nila ang repo. Niru-run nila ang `npm install`. May nasisira. 45 minutos later, nagde-debug na sila ng Ruby version mismatch, missing na CocoaPod, o Node version na hindi tumatakbo sa bundler. Pag-run na ng app, ubos na ang pasensya nila at kalahati na ng gabi nila.
...
Ang tech test ang unang totoong interaction ng kandidato sa engineering culture mo. Lahat ng nahihipo niya, may sinasabi tungkol sa iyo.
...
Kaya tine-treat ko ang test na katulad ng pag-treat ko sa product:
...
Pagkatapos ng Ruby incident, nag-add ako ng setup script, ni-pin ko ang Ruby version, nag-commit ng Gemfile.lock na may modern bundler, at nagdagdag ng troubleshooting section sa README. Ang susunod na kandidato, nagco-code na sa loob ng dalawang minuto.

## Ang setup script

Ang pinaka-malaking improvement ay isang `setup.sh` na nag-hahandle ng lahat.
...
I-pin mo ang versions sa repo. `.ruby-version`, `.nvmrc`, `Gemfile.lock` na may modern bundler. Tapos gumawa ka ng setup script na nagba-basa sa kanila. Bawat minutong ginugol ng kandidato sa setup, isang minuto na hindi niya ginugugol sa code.
...
May mga kandidato na nagsa-start from scratch kahit na meron ang starter. Okay lang yun, optional ang starter. Pero karamihan ginagamit ito, at sa halip na gugulin ang unang oras sa paglaban sa config, ginugugol nila sa pag-gawa ng architectural decisions.

## Ang brief: malinaw sa kung ano, hindi sa kung paano

May mga tech test na ini-specify ang exactly kung paano gawin ang mga bagay. Kung anong state management library, kung anong folder structure, kung anong API client. Gumagana ang approach na yun kapag kailangan mo ng magkakatulad na submissions, o kapag ang role ay talagang tungkol sa pagta-trabaho sa loob ng mahigpit na conventions. Para sa amin, yung mga decision na yun ang pinaka-interesting na parte ng submission, kaya kapag ni-lock down namin ang mga ito, itatapon namin ang signal na gusto namin talaga.

Kabaligtaran ang approach ng brief namin. Ine-explain nito nang detalyado kung ano ang dapat gawin ng app, at walang sinasabi tungkol sa kung paano.
...
Kung ang brief mo ay nags-specify ng architecture, compliance ang tine-test mo, hindi engineering. Ang mga briefs na gumagana para sa amin ay nagde-describe ng ano nang detalyado at iniiwang bukas nang buo ang paano.
...
Honest ang 4 hanggang 6 na oras na estimate. Ginawa ko mismo ang test para i-check. Kaya ng isang competent na React Native developer na i-build ang lahat ng tatlong screens na may state management, API integration, basic tests, at README sa oras na yun. May mga pumipili na mag-invest ng mas marami. Choice nila yun, hindi expectation namin.

Kung kailangan ng kandidato ng mas maraming oras, binibigay namin. Walang tanong. Iba ang signal ng pagiging tahimik at mag-submit tatlong araw late na walang explanation kumpara sa pagse-send ng message na "kailangan ko ng konting dagdag na oras." Importante ang communication.

## Sabihin mo kung ano ang hinahanap mo

Noong una, isang kandidato ang gumugol ng isang oras sa pag-style ng buttons kasi akala niya mahalaga sa amin ang UI polish. Hindi pala. Architecture at testing ang tinitingnan namin. Na-waste ang oras na yun para sa amin, hindi para sa kanya, kasi hindi namin sinabi kung ano ang importante.
...
Sinasabi rin namin sa kanila upfront na pwedeng gumamit kami ng automated tooling bilang pre-check, pero bawat submission ay manually reviewed at scored ng hiring panel. Ang transparency ay nagbu-build ng trust.

## Ang walkthrough ay conversation

Ang kandidato ang nangunguna sa unang 10 minuto:
...
Pagkatapos ng presentation, nagta-tanong kami. Ang framing ang mahalaga. Sinasabi namin:
...
Signal ang punto, hindi pagiging magalang lang. Ang panonood sa isang tao na nagda-diagnose ng bug sa sarili niyang code ay isa sa pinakamalakas na reads na pwede mong makuha sa isang engineer. Isang kandidato na nagsabing *"Ah, parang mali yata ang useEffect dependency array dito"* ay nagpapakita sa iyo kung paano talaga siya nagta-trabaho.
...
Bawat tanong na pwedeng maisip ng kandidato, nasagot na bago pa niya kailanganin itanong. Ang goal ay alisin ang ambiguity bilang variable. Ayaw kong i-evaluate kung gaano kahusay mag-interpret ng vague brief ang isang tao. Gusto kong i-evaluate kung paano sila nagbu-build ng software kapag malinaw ang requirements.

## Ano ang babaguhin ko sa susunod

Hindi pa tapos ang test. Ito ang nasa listahan ko:
...
- Isang `.env.example` file. Gumagamit ang test ng public API na walang keys, pero ang pag-commit ng file ay nag-sset ng tamang pattern.
- I-run ang setup sa clean machine bago bawat hiring round. Nag-iipon ng taon-taon na tooling ang developer laptop, at ang kahit anong assumption na "lahat may ganito naka-install" ay nasisira sa sandaling i-run ng kandidato ang script sa fresh OS install.
