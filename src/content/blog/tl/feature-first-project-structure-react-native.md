...
Bina-block ng unang pattern ang kahit anong isang level sa loob ng feature (`@app/features/Auth/store`), at ng pangalawa ang mas malalim pa; ang bare na `@app/features/Auth` index import ay hindi tumutugma sa alinman, kaya nananatiling bukas ang public surface. Isang trap na sulit malaman: gumagamit ang mga pattern na ito ng gitignore-style matching, hindi extglob, kaya ang nakakatuksong `!(index)` exclusion ay tahimik na walang tinutugmaan. Sa loob ng feature, gumagamit ka ng relative imports (`./store`, `../components`), na hindi kailanman tumutugma sa alias pattern, kaya laging kayang abutin ng feature ang sarili nitong code. Ang tanging exemption ay ang tests, na madalas nangangailangang umabot sa loob ng feature para mag-set up ng state.
...
At kung gusto mo ang one-line version ng buong post na ito: buksan ang `screens/` folder mo, bilangin ang files, at itanong kung gaano katagal aabutin ang pag-delete ng isang feature ngayon.
