...
Clonan el repo. Corren `npm install`. Algo se rompe.

**45 minutos después**, están debuggeando un mismatch de versión de Ruby, un CocoaPod faltante o una versión de Node que no funciona con el bundler. Para cuando la app corre, ya quemaron su paciencia y la mitad de su noche.

Los mejores candidatos, los que realmente querés contratar, son los que más probabilidades tienen de irse. Tienen opciones. Van a elegir la empresa que respete su tiempo.
...
## Tratá la prueba como un producto

Este se volvió mi principio rector. La prueba técnica es la primera interacción real que un candidato tiene con tu cultura de ingeniería. Todo lo que experimenta le dice algo sobre vos.
...
Después del incidente con Ruby, agregué un script de setup, fijé la versión de Ruby, commiteé un Gemfile.lock con un bundler moderno y agregué una sección de troubleshooting al README.
...
- ✅ Chequea la versión de Node (instala via nvm si hace falta)
- ✅ Chequea la versión de Ruby (soporta rbenv, rvm y asdf)
- ✅ Chequea Xcode CLI tools y CocoaPods
- ✅ Corre `yarn install`
- ✅ Corre `bundle install` y `pod install`
- ✅ Te dice exactamente qué arreglar si algo anda mal
...
> 💡 **Tip:** Fijá las versiones en el repo: `.ruby-version`, `.nvmrc`, `Gemfile.lock` con un bundler moderno. Después escribí un script de setup que las lea. Cada minuto que un candidato pasa en el setup es un minuto que no está escribiendo código.
...
| Wrapper de render custom para tests | NavigationContainer incluido, solo renderizá y hacé assert |
...
**Todo compila. Todo corre. El smoke test pasa.**
...
Algunos candidatos arrancan de cero igual. Está bien. El starter es opcional. Pero la mayoría lo usa, y el resultado es el mismo: en vez de pasar su primera hora peleando con configuración, la pasan tomando decisiones de arquitectura.

## El brief: claro en el qué, no en el cómo

Algunas pruebas técnicas especifican exactamente cómo construir las cosas: qué librería de state management, qué estructura de carpetas, qué cliente de API. Ese enfoque funciona cuando querés consistencia. Pero para nosotros, esas decisiones son la parte más interesante de la entrega.
...
- **Una tabla de requisitos** detalla las reglas (máximo 6 items, agregar desde el detalle, eliminar desde la lista)
...
**Los candidatos tienen 7 días. El trabajo debería tomar de 4 a 6 horas.**

Lo decimos explícitamente. En el brief y en la guía de entrega. Dos veces, porque la gente se lo pierde la primera.

7 días da flexibilidad. Algunos trabajan durante un fin de semana. Algunos hacen una hora cada noche. Algunos bloquean un sábado a la mañana. El timeline respeta que los candidatos tienen trabajos, familias y una vida fuera del proceso de entrevistas.
...
```
✅ Cómo pensás la arquitectura y la organización del código
✅ Cómo descomponés un problema en componentes y flujos de datos
✅ Cómo tomás y justificás decisiones técnicas
✅ Cómo manejás casos borde y estados de error
✅ Qué tan bien conocés tu propio código
...
2. **Correr los tests**: mostrarlos pasando en vivo
...
> *"No te preocupes si algo no funciona como esperabas durante la demo. Pasa. Si pasa, contame qué pensás que falló y cómo lo arreglarías. Eso me dice más que una demo perfecta."*

No es solo ser amable. Ver a alguien diagnosticar un bug en su propio código es una de las señales más fuertes que podés obtener. Un candidato que dice *"Ah, creo que el dependency array del useEffect está mal acá"* te está mostrando exactamente cómo trabaja.
...
- **Un video walkthrough del proyecto starter.** Un Loom de 3 minutos mostrando la estructura de carpetas, cómo correrlo y por dónde empezar. Algunas personas aprenden mejor con video que con docs.
- **Un archivo `.env.example`.** Aunque la prueba usa una API pública sin keys, establece el patrón correcto.
- **Probar el setup en una máquina limpia.** Construí la prueba en mi propia laptop con años de herramientas instaladas. Cada suposición sobre "todos tienen esto instalado" estaba equivocada. El primer candidato lo demostró.

La estructura está bien igual. Script de setup. Proyecto starter. Brief claro. Timeline honesto. Documentación de verdad. Criterios de evaluación transparentes.

Si estás diseñando una prueba técnica y los candidatos siguen abandonando, no mires las preguntas primero. Mirá la experiencia de desarrollo. **La mejor prueba técnica es una donde el candidato pasa el 100% de su tiempo en lo que realmente estás evaluando, y 0% en todo lo demás.**
