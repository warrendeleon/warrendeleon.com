...
Dos personas revisaron la misma entrega. Uno le puso un 3 al TypeScript ("hay tipos pero no son estrictos"). El otro le puso un 4 ("tipos limpios en todo el código, buen uso de hooks tipados"). Estaban mirando el mismo código. Simplemente leyeron la rúbrica de forma distinta. Si dos personas razonables pueden no estar de acuerdo en la puntuación, la rúbrica no es lo suficientemente específica. El problema no son los revisores, es la herramienta.


## Checklists en vez de rúbricas

La solución fue simple: reemplazar cada puntuación subjetiva por un checklist de sí/no.
...
Los primeros checks de cada criterio son cosas que cualquier candidato competente va a sacar en 4 a 6 horas:
...
Estas no son cosas que haces en un fin de semana. Son patrones que recoges enviando apps reales a producción.

Un candidato que invierte 4 a 6 horas saca entre 50 y 65. Un candidato que invierte una semana entera con años de oficio detrás puede sacar 85 a 95. **El brief es el mismo. Las expectativas escalan con la puntuación.**
...
Una prueba técnica no es una app de producción. Los candidatos tienen trabajos, familias, vidas. Te están dando su noche o su fin de semana. Bajarle la nota a alguien por no implementar una capa de caché o por no co-localizar sus estilos sería como bajarle nota a un ensayo con tiempo por no tener notas al pie.

Por eso importan los checks de línea base. Tenerlos todos bien te da alrededor de 50 a 65 de 100. Eso es territorio de Associate a Software Engineer. En mi vieja rúbrica, un "3 de 5" *sonaba* como premio de consolación. 55 de 100 en el checklist es un resultado positivo con un camino claro al siguiente nivel.
...
Un candidato que añade tests E2E con Detox con helpers extraídos me está diciendo algo sobre su cultura de testing. Uno que implementa GraphQL con Apollo me está diciendo algo sobre cómo piensa las APIs. Uno que configura MSW con múltiples conjuntos de handlers (éxito, error, 401, timeout, offline) me está diciendo que ya debuggeó fallas de API reales antes.

Nada de esto es obligatorio. Todo se nota.

Los stretch goals se suman a los 100 puntos como bonificaciones: búsqueda, modo oscuro, accesibilidad, i18n, estructura feature-first, Storybook, ErrorBoundary. Son las marcas de alguien que tuvo tiempo y eligió gastarlo bien.
...
Algunas lecciones que me quedaron:

Empieza con checklists, no con rúbricas. Cada vez que escribía una rúbrica ("5 = excelente, 3 = bueno, 1 = malo"), se convertía en un debate sobre qué significa "bueno". Los checklists terminan el debate. El criterio está en el código o no está.

Ordena los checks por inversión, no por importancia. Los primeros checks no son más importantes que los últimos. Solo son más alcanzables en 4 a 6 horas. Un candidato Senior que se salta el check 3 pero clava el check 7 no se penaliza por el salto porque el total sigue reflejando su nivel.
