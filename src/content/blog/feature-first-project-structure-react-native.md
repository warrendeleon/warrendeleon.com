...
heroImgPrompt: "Several plain closed cubes arranged in a neat ring around one larger central cube, each small cube with a single doorway opening, thin connectors running from the centre to each"
...
```mermaid
flowchart TD
    subgraph features["Features (private behind index.ts)"]
        A[Auth]
        B[Profile]
        C[Settings]
    end
