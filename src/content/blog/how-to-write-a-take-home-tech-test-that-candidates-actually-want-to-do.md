...
They clone the repo. They run `npm install`. Something breaks. Forty-five minutes later they're debugging a Ruby version mismatch, a missing CocoaPod, or a Node version that doesn't play with the bundler. By the time the app runs they've burnt through their patience and half their evening.
...
The tech test is the first real interaction a candidate has with your engineering culture. Everything they touch tells them something about you.
...
So I treat the test the same way I'd treat a product:
...
After the Ruby incident I added a setup script, pinned the Ruby version, committed a Gemfile.lock with a modern bundler, and added a troubleshooting section to the README. The next candidate was coding in under two minutes.

## The setup script

The single biggest improvement was a `setup.sh` that handles everything.
...
Pin your versions in the repo. `.ruby-version`, `.nvmrc`, `Gemfile.lock` with a modern bundler. Then write a setup script that reads them. Every minute a candidate spends on setup is a minute they're not spending on code.
...
Some candidates start from scratch anyway. That's fine, the starter is optional. Most use it, though, and instead of spending their first hour fighting config they spend it making architectural decisions.

## The brief: clear about what, not how

Some tech tests specify exactly how to build things. Which state management library, which folder structure, which API client. That approach pays off when you need apples-to-apples submissions, or when the role really is about working within tight conventions. For us, those decisions are the most interesting part of the submission, so locking them down would throw away the signal we actually want.

Our brief takes the opposite approach. It explains what the app should do in detail and says nothing about how.
...
A candidate who picks Redux Toolkit tells me something different from one who picks Zustand. Neither is wrong. Both are interesting. The reasoning behind the choice is what the walkthrough conversation is built on.

If your brief specifies the architecture, you're testing compliance, not engineering. The briefs that work for us describe the what in detail and leave the how completely open.

## Respecting people's time

Candidates get 7 days. The work should take 4 to 6 hours.

We say that explicitly, in the brief and in the submission guide, twice, because people miss it the first time.
...
The 4-to-6-hour estimate is honest. I built the test myself to check it. A competent React Native developer can build all three screens with state management, API integration, basic tests, and a README in that time. Some choose to invest more. That's their call, not our expectation.

If a candidate needs more time, we give it. No questions asked. Going silent and submitting three days late with no explanation is a different signal from sending a message saying "I need a couple more days." Communication matters.

## Tell them what you're looking for

Early on, a candidate spent an hour styling buttons because they'd assumed UI polish mattered to us. It didn't. We were looking at architecture and testing. That hour was wasted on our behalf, not theirs, because we hadn't told them what counted.
...
When candidates know we care about architecture and trade-offs more than styling, they spend their time accordingly. Better signal for us, better experience for them.

We also tell them upfront that we might use automated tooling as a pre-check, but every submission is manually reviewed and scored by the hiring panel. Transparency builds trust.

## The walkthrough is a conversation

The candidate drives for the first 10 minutes:
...
After the presentation, we ask questions. The framing matters. We say:
...
The point is signal, not politeness. Watching someone diagnose a bug in their own code is one of the strongest reads you can get on an engineer. A candidate who says *"Oh, I think the useEffect dependency array is wrong here"* is showing you exactly how they work.
...
Every question a candidate might have is answered before they need to ask it. The goal is to take ambiguity off the table. I don't want to evaluate how well someone interprets a vague brief. I want to evaluate how they build software when the requirements are clear.

## What I'd change next time

The test isn't finished. What's on my list:
...
- A `.env.example` file. The test uses a public API with no keys, but committing the file sets the right pattern.
- Running the setup on a clean machine before every hiring round. A developer laptop accumulates years of tooling, and any assumption that "everyone has this installed" tends to break the moment a candidate runs the script on a fresh OS install.
